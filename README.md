...
suitable to create a report in the shape of an html page. This html page
contains all plots created by means of the SMASH-analysis suite, sorted by
observables. It can be produced by executing the `create_report.sh`
...
references to the config files.


## Storing the results for future comparison

If not disabled, all generated results are compared to those of the previous
...
data files, `old_results_SMASH-{smash-version}`, is then created within the
